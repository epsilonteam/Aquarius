...
object ColorUtils {
    ...
    inline fun rainbowHSB(delay: Int): ColorHSB {
        ...
        rainbowState %= 360.0
        return ColorHSB((rainbowState / 360.0f).toFloat(), GUIManager.saturation, GUIManager.brightness)
    }

    inline fun rainbowRGB(delay: Int): ColorRGB = rainbowHSB(delay).toRGB()

    val IChildComponent.startColor get() = if (GUIManager.isRainbow) rainbowRGB((father.y - y) * 5) else GUIManager.color
    val IChildComponent.endColor get() = if (GUIManager.isRainbow) rainbowRGB((father.y - y + height) * 5) else GUIManager.color

    val IChildComponent.startColor2 get() = if (GUIManager.isRainbow) rainbowRGB(((father as IChildComponent).father.y - y) * 5) else GUIManager.color
    val IChildComponent.endColor2 get() = if (GUIManager.isRainbow) rainbowRGB(((father as IChildComponent).father.y - y + height) * 5) else GUIManager.color
